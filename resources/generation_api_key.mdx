...
The CentML platform allows you to generate and manage `Vault` objects such as `Bearer Tokens`, `Certificates`, `Registry Credentials`, `Environment Variables`, `Hugging Face Tokens`, and `SSH Keys`.
...
You will need to login to the CentML platform to manage Vault objects. If you do not know how to login or do not have an account, please follow the [Creating an Account](/resources/account_creation) documentation before proceeding. 
...
Registry credentials allow the CentML Platform to pull container images from private registries when creating [General Inference](/apps/inference) deployments.
...
Supported registries include Docker Hub, Amazon ECR, Google Artifact Registry, Azure Container Registry, and any Docker-compatible private registry. The platform automatically detects the registry from the image URL at deployment time.
