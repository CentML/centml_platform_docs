...
description: 'How to build AI agents using structured outputs such as JSON and function/tool calls with NVIDIA CCluster'
icon: 'user-secret'
---
...
Below is an example of how to set up a request to the NVIDIA CCluster LLM API using JSON schema enforcement.
...
# Supply your API key from your NVIDIA CCluster vault
...
# Initialize the OpenAI-compatible client with the API key and base URL
client = OpenAI(
    api_key=api_key,
...
# Initialize the OpenAI-compatible NVIDIA CCluster client with the API key and base URL
...
<Note>
**Tip**: Make sure your model (e.g., `gpt-4o` or `deepseek-ai/DeepSeek-R1`) is function-call capable. Not all model versions support function calling. Always check the NVIDIA CCluster model documentation for compatibility.
</Note>
...
Using JSON schema enforcement and function calling (tools) with NVIDIA CCluster LLM APIs lets you build more reliable, agentic, and automated workflows. With minimal changes to your existing OpenAI-compatible code, you can take advantage of these features on the NVIDIA CCluster.
