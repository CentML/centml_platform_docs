---
title: 'Python SDK Reference'
description: 'Access the CentML CCluster using python'
icon: 'python'
---

We provide a Python SDK to programmatically access CentML CCluster backend APIs to manage your deployments.

Following is a sample code to list all the inference deployments under your account.

```python
import centml
from centml.sdk.api import get_centml_client
from centml.sdk import DeploymentType

with get_centml_client() as cclient:
    deployments = cclient.get(DeploymentType.INFERENCE_V2)
    print(deployments)
```

For more examples, please refer to https://github.com/CentML/centml-python-client/blob/main/examples/sdk/


## What's Next

<CardGroup cols={2}>
  <Card
    title="Managing Vault Objects"
    icon="key-skeleton-left-right"
    href="/resources/generation_api_key"
  >
    Learn how to generate and store CentML tokens and other vault objects.
  </Card>
  <Card
    title="Clients"
    icon="terminal"
    href="/clients/setup"
  >
    Learn how to interact with the CentML CCluster programmatically
  </Card>
  <Card
    title="Resources and Pricing"
    icon="coins"
    href="/resources/pricing"
  >
    Learn more about CentML Pricing
  </Card>
    <Card
    title="Get Support"
    icon="headset"
    href="/resources/requesting_support"
  >
    Submit a Support Request
 </Card>
  <Card
    title="Quickstart"
    icon="rocket-launch"
    href="/home/quickstart"
  >
    Get started with the CentML CCluster in minutes.
 </Card>
  <Card
    title="The Model Integration Lifecycle"
    icon="arrows-spining"
    href="/resources/model_integration_lifecycle"
  >
    Dive into how CentML can help optimzie your Model Integration Lifecycle (MILC). 
 </Card>
</CardGroup>
