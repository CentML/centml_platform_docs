...
- **registry credentials:** If your container image is hosted on a private registry, provide credentials to allow the platform to pull the image. You can select stored credentials from your [Vault](/resources/generation_api_key) or enter a registry username and password directly. The registry is automatically detected from the image URL.
