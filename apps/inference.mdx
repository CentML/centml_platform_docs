...
We make it easy for you to containerize and deploy any custom models on our platform. Containers can run as **any user, including root** — see [Deploying Custom Models](/resources/custom_image) for platform security details. We also provide popular inference engine recipes such as vLLM, Ollama, SGLang, and LMDeploy.
...
- **Health check path** — the endpoint used to verify readiness (e.g., `/health`, `/`). For gRPC deployments, the platform uses TCP socket checks automatically, so this field is ignored.
- **Image Registry Username / Password** — required only if the image is hosted on a private registry (e.g., Docker Hub private repo). You can also use credentials stored in your [Vault](/resources/generation_api_key).
...
- **Autoscaling** — set the min and max scale for your deployment. The platform scales replicas based on max concurrency (maximum in-flight requests per replica). Default is infinity.
...
The platform provides pre-configured recipes for popular inference engines in the **Add or Select Image** dropdown. Selecting a recipe auto-fills all fields except the entrypoint command. Below are the **Add command** values you need to provide for each engine:
...
- **Ollama** — leave **Add command** empty. After the deployment is ready, pull a model before querying:
