...
[NVIDIA NIM](https://developer.nvidia.com/nim) microservices are self-contained containers that bundle a model-optimized inference engine and download model weights automatically at startup, exposing an OpenAI-compatible API. NIM is not a pre-configured recipe — enter the image manually with the settings below.

NIM images are hosted on `nvcr.io`. Browse the catalog at [build.nvidia.com](https://build.nvidia.com/explore/discover) and copy the image path from the model's **Deploy** tab, for example:
...
Every NIM uses the same configuration:

| Field | Value |
|---|---|
| **Add or Select Image** | The `nvcr.io/nim/...` image path (entered manually) |
| **Container Port** | `8000` |
| **Protocol** | `HTTP` |
...
Once the deployment is ready, query it like any OpenAI-compatible endpoint. List the served model identifier first, then send requests:
