...
We make it easy for you to containerize and deploy any custom models on CCluster. Containers can run as **any user, including root** — see [Deploying Custom Models](/resources/custom_image) for CCluster security details. We also provide popular inference engine recipes such as vLLM, Ollama, SGLang, LMDeploy, and NVIDIA NIM.
...
- **NVIDIA NIM** — leave **Add command** empty. See the NIM-specific note below.
...
<Note>
**NVIDIA NIM** containers bundle a model-optimized inference engine and download weights automatically at startup. No entrypoint command is needed, but NIM requires three specific settings:

| Field | Value |
|---|---|
| **Container Port** | `8000` |
| **Health check path** | `/v1/health/ready` |
| **Image Registry Username** | `$oauthtoken` (literal string) |
| **Image Registry Password** | Your [NGC API Key](https://ngc.nvidia.com) |

Under **Optional Details → Environment Variables**, also set:

| Key | Value |
|---|---|
| `NGC_API_KEY` | Your NGC API Key |

The container uses `NGC_API_KEY` to download model weights on first boot — without it the deployment will start but fail to load the model. NIM images are hosted on `nvcr.io`, for example:
...
Browse the full NIM catalog at [build.nvidia.com](https://build.nvidia.com/explore/discover). Once the deployment is `ACTIVE`, NIM exposes an OpenAI-compatible API:
