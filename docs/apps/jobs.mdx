...
<Note>
The environment variable names `JOB_COMPLETION_INDEX` and `JOB_COMPLETIONS` are reserved by the platform and cannot be used for your own variables. When you run a multi-completion job, `JOB_COMPLETION_INDEX` identifies each pod's index.
</Note>
