---
title: Quickstart
description: Harness the power of AI in a few minutes
icon: rocket
---

This quickstart shows you how to integrate large langauge models (LLMs) in to your applications using CentML Platform in three easy steps.

## 1. Create an account and login
Go to https://app.centml.com and create your CentML user account. Afterwards, login to the CentML Platform console.

<Frame>
    <img src="/images/homepage.png" style={{ borderRadius: '0.5rem' }} />
</Frame>

## 2. Create an API key
Go to Account -> Vault and generate an API key.

<Frame>
    <img src="/images/api_token.png" style={{ borderRadius: '0.5rem' }} />
</Frame>

## 3. Start using the LLM through our serverless endpoints

Select the Serverless option from the menu and start conversing with your preferred model through the chat UI.

<Frame>
    <img src="/images/chat.png" style={{ borderRadius: '0.5rem' }} />
</Frame>

In addition to the chat UI, serverless endpoint can also be accessed programmatically through Python, Javascript, and cURL command. Switch to the API tab to find the corresponding code snippets to get started.

<Frame>
    <img src="/images/api.png" style={{ borderRadius: '0.5rem' }} />
</Frame>

### What next?

<CardGroup cols={2}>
  <Card
    title="Our Services"
    icon="box-open-full"
    href="/apps/llm"
  >
    Learn how to build your AI application using CentML Platform
  </Card>
  <Card
    title="Clients"
    icon="terminal"
    href="/clients/setup"
  >
    How to interact with Platform programmatically
  </Card>
  <Card
    title="Resouces and Pricing"
    icon="coins"
    href="/resources/pricing"
  >
    Learn more about CentML Platform
  </Card>
</CardGroup>
